...
Run from sumo_3/:
    ...
    python3 script/run_simulation.py --workers 8      # limit to 8 parallel
...
def run_day(day: date, dry_run: bool = False) -> tuple[str, bool, str]:
    """
    Run SUMO for one day. Returns (date_str, success, message).

    Uses --output-prefix to redirect all relative output paths (including the
    detector XML output) directly into the dated directory, avoiding any
    conflict when multiple days run in parallel.
    """
    ...
    # Collect summary stats from stdout
    summary_lines = [
    ...
    if os.path.exists(det_file):
        ...
        summary_lines.append(f"detector_output.xml: {size_mb:.1f} MB")
    else:
